...
Actionable wakes include captain-relevant status signals, no-verb signals without positive evidence that their crew is still executing, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes with no open keyed status decision that persist past `FM_STALE_ESCALATE_SECS` without their own task worktree being written, declared external waits and verified captain-held transfers that remain declared past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
The wedge timer itself also consults the status-log decision fold (`status_has_open_decision` in `bin/fm-classify-lib.sh`) before firing: a pane parked on an unanswered keyed `needs-decision` or `blocked` is waiting for firstmate, so that idle window restarts instead of escalating, and closing the decision restores the unchanged schedule.
...
The fold remains the sole owner of open/closed semantics, including same-key reopening and reserved-key handling, shared with the durable OPEN DECISIONS surface and with wedge suppression, which consults that same fold rather than the last status line.
...
The same override also leaves an unanswered keyed decision un-escalated, because that fold already records that the pane is waiting for firstmate.
...
Away-mode housekeeping has no worktree-write deferral of its own, so while `state/.afk` exists a quiet crew that is writing its own worktree and has neither a declared wait nor an unanswered keyed decision still escalates as a possible wedge at that bound.
