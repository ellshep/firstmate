...
The wedge timer also consults the status-log decision fold (`status_has_open_decision` in `bin/fm-classify-lib.sh`) right after that declared-wait check: a pane parked on an unanswered keyed `needs-decision` or `blocked` is waiting for firstmate, so that idle window restarts instead of escalating, and closing the decision restores the unchanged schedule.
The two are disjoint by verb, and a verified captain-held transfer closes the key in that fold rather than opening one, so neither defers a pane the other already claimed.
