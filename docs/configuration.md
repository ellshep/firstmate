...
`bin/fm-claude-calm-display.sh` reads that same file, with the same values and the same resolution order, so one choice covers both harnesses.
...
The extension reloads this preference on every Pi `session_start`, including startup, new, resume, fork, and reload reasons, and the Claude Code hook reads it fresh at each displayed message.
