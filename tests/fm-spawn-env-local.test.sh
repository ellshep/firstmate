...
KEPT_KEYS='APP_NAME SUPABASE_URL SUPABASE_SERVICE_KEY MY_PRODUCT USER PORT user port PUBLIC_URL API_PORT SMTP_HOST SMTP_USER SMTP_PASSWORD WEB_HOST ADMIN_USER SENTRY_DSN NEXT_PUBLIC_API_HOST STRIPE_SECRET_KEY REDIRECT_URL'
DROPPED_KEYS='DATABASE_URL DATABASE_URI DATABASE_URL_POOLED DATABASE_URL_DIRECT DATABASE_URL_PROD POSTGRES_URL POSTGRESQL_URL INTERNAL_DB DB_HOST DB_USER DB_PASSWORD MYSQL_HOST PGHOST PGHOSTADDR PGPORT PGDATABASE PGUSER PGPASSWORD PGPASSFILE PGSERVICE PGSERVICEFILE APP_DB_HOST SUPABASE_DB_CONNECTION MSSQL_URL SQLSERVER_URL COCKROACH_URL SERVICE_CONNECTION SERVICE_ENDPOINT SUPABASE_SERVICE_KEY_PROD CONNECTION_PG_DRIVER CONNECTION_MY_DRIVER CONNECTION_BARE CONNECTION_QUOTED'
VENDOR_TOKENS='MYSQL MARIADB MSSQL SQLSERVER SQLSRV COCKROACHDB COCKROACH CRDB MONGODB MONGO REDIS VALKEY POSTGRES POSTGRESQL'
...
REDIRECT_URL=https://example.test/?next=postgres://example.test/db
CONNECTION_PG_DRIVER=postgresql+psycopg://hosted-db/app
CONNECTION_MY_DRIVER=mysql+pymysql://hosted-db/app
CONNECTION_BARE=postgres://hosted-db/app
CONNECTION_QUOTED="postgres://hosted-db/app"
